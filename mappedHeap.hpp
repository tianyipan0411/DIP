...
public:
	void Check() {
		...
		cout <<"Testing heap...";
		if( mh.top() != 2) cout<<"Error: MappedHeap constructor test failed!" <<endl;
		mh.push(1);
		if (mh.top() != 1) cout<<"Error: MappedHeap push test failed!" <<endl;
		value[4] = 0;
		mh.heapify(4);
		if(mh.top() != 4) cout<<"Error: MappedHeap heapify test failed!" <<endl;
		mh.pop();
		if( mh.top() != 1) cout<<"Error: MappedHeap pop test failed!" <<endl;
		cout <<"..done"<<endl;
	}
};
